...
import HmiCard from '../../components/HmiCard';
import { fetchHmiList } from '../../helpers/device'; // 這個 API 會一次返回所有資料
import { theme } from '../../theme/theme';

export default function HmiListScreen() {
  ...
  const loadData = useCallback(() => {
    ...
    fetchHmiList()
      .then((list) => {
        ...
        setFilteredData(list); // 初始時，過濾資料等於完整資料
        setSearch(''); // 重新整理後清空搜尋框
      })
      .catch((err) => {
        console.error("Failed to fetch HMI list:", err);
  ...
  }, [search, originalData]); // 當搜尋文字或原始資料改變時觸發
